...
<%-- 
	revel 나의 댓글 목록 페이지
	http://localhost:8080/revel/mypage/review.rv 
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <!-- 해당 페이지 이름 -->
                    <h1 class="h3 mb-2 text-gray-800">My Review</h1>

                    <!-- DataTales Example -->
                    <!-- 나의 댓글 목록 테이블 -->
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">나의 댓글 목록</h6>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                            	<!-- count가 0이면 내가 쓴 댓글이 없다. -->
                            	<c:if test="${count == 0}">
                            		<p align="center">내가 쓴 댓글이 없습니다.</p>
                            	</c:if>
                            	
                            	<!-- count가 1이상이면 저장된 내가 쓴 댓글을 보여준다. -->
                            	<c:if test="${count > 0}">	
                                    	...
                                    	<tbody align="center">
                                    		<c:forEach var="myReview" items="${myReviewList}">
                                        		<tr>
                                            		...
                                            		<td>
                                            			<a href="/revel/detail.rv?bookNum=${myReview.index_book}">
                                            				${myReview.title}
                                            			</a>
                                            		</td>
                                            		<td>
                                            			<c:if test="${myReview.cleanbot == 0}">
                                            				${myReview.content}
                                            			</c:if>
                                            			<c:if test="${myReview.cleanbot > 0}">
                                            		...
                                            		<td>
                                            			<fmt:formatDate value="${myReview.reg_time}" pattern="yyyy-MM-dd HH:mm" />
                                            		</td>
                                            		<td>
                                            			<a href="/revel/mypage/reviewDel.rv?reviewNum=${myReview.index_review}&bookNum=${myReview.index_book}" 
                                            			   class="btn btn-danger btn-circle btn-sm" onclick="return myReviewDel();">
                                ...
                                <!-- 삭제 최종확인 알림창 -->
                                <script type="text/javascript">
                                	function myReviewDel()
                                	{
                                		var msg = confirm("댓글을 삭제하시겠습니까?");
                                		if(msg)
                                		{
                                			// yes
                                			widow.location = "/revel/mypage/reviewDel.rv";
                                		}
                                		else
                                		{
                                			// no
