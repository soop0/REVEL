...
<%-- 
	월간 TOP20페이지
	http://localhost:8080/revel/ranking/monthly.rv 
--%>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <span onclick="location.href='/revel/ranking/monthly.rv'" style="cursor:pointer;">Monthly</span>
                    </div>
                </div>
            </div>
        </div>
                        		...
                        		<c:forEach var="monthly" items="${monthly}" varStatus="status">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${monthly.image}" onclick="location.href='/revel/detail.rv?bookNum=${monthly.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${monthly.comple == 1}">완결</c:if> <c:if test="${monthly.comple == 0}">연재중</c:if> / ${monthly.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${monthly.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${monthly.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                        			<li style="background-color: SeaGreen">${status.count}위</li>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${monthly.genre == 1}">
                                            				<c:set var="gen" value="romance"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">로맨스</a>
                                            			</c:if>
                                            			<c:if test="${monthly.genre == 2}">
                                            				<c:set var="gen" value="rofan"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">로맨스판타지</a>
                                            			</c:if>
                                            			<c:if test="${monthly.genre == 3}">
                                            				<c:set var="gen" value="fantasy"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">판타지</a>                                            				<c:set var="gen" value="fantasy"/>
                                            			</c:if>
                                            			<c:if test="${monthly.genre == 4}">
                                            				<c:set var="gen" value="mofan"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">현대판타지</a>
                                            			</c:if>
                                            			<c:if test="${monthly.genre == 5}">
                                            		...
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == monthly.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${monthly.index_book}">${monthly.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
