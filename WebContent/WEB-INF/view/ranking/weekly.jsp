...
<%-- 
	주간 TOP20페이지
	http://localhost:8080/revel/ranking/weekly.rv 
--%>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <a href="/revel/ranking/daily.rv">Ranking</a>
                        <span onclick="location.href='/revel/ranking/weekly.rv'" style="cursor:pointer;">Weekly</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="product__page__content">
                        ...
                        <c:if test="${count > 0}">
                        	<div class="row">
                        		<c:forEach var="weekly" items="${weekly}" varStatus="status">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${weekly.image}" onclick="location.href='/revel/detail.rv?bookNum=${weekly.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${weekly.comple == 1}">완결</c:if> <c:if test="${weekly.comple == 0}">연재중</c:if> / ${weekly.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${weekly.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${weekly.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                        			<li style="background-color: SeaGreen">${status.count}위</li>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${weekly.genre == 1}">
                                            				<c:set var="gen" value="romance"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">로맨스</a>
                                            			</c:if>
                                            			<c:if test="${weekly.genre == 2}">
                                            				<c:set var="gen" value="rofan"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">로맨스판타지</a>
                                            			</c:if>
                                            			<c:if test="${weekly.genre == 3}">
                                            				<c:set var="gen" value="fantasy"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">판타지</a>                                            				<c:set var="gen" value="fantasy"/>
                                            			</c:if>
                                            			<c:if test="${weekly.genre == 4}">
                                            				<c:set var="gen" value="mofan"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">현대판타지</a>
                                            			</c:if>
                                            			<c:if test="${weekly.genre == 5}">
                                            		...
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == weekly.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${weekly.index_book}">${weekly.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
