...
<%-- 
	revel 검색결과 페이지
	http://localhost:8080/revel/genre/search.rv 
--%>
                        		...
                        		<c:forEach var="search" items="${searchList}">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${search.image}" onclick="location.href='/revel/detail.rv?bookNum=${search.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${search.comple == 1}">완결</c:if> <c:if test="${search.comple == 0}">연재중</c:if> / ${search.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${search.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${search.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${search.genre == 1}">로맨스</c:if>
                                            			<c:if test="${search.genre == 2}">로맨스판타지</c:if>
                                            			<c:if test="${search.genre == 3}">판타지</c:if>
                                            			<c:if test="${search.genre == 4}">현대판타지</c:if>
                                            			<c:if test="${search.genre == 5}">무협</c:if>
                                            		</li>
                                            		<!-- 키워드 나타내기 -->
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			<c:set var="keyVal" value="${keywordMap.key}" />
                                            			<c:if test="${keyVal == search.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${search.index_book}">${search.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                    ...
                    <c:if test="${count > 0}">
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/search.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/search.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/search.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
