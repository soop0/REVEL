...
<%-- 
	revel 공지사항 수정 페이지
	http://localhost:8080/revel/help/notice/update.rv 
                ...
                <div class="col-lg-8">
                    <div class="product__page__content">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-6">
                                    <div class="product__page__filter">
                                      <c:if test="${sessionScope.status == 100}">	
                                      	<a href="#" onClick="history.back()"><p>이전으로</p></a>
                                      </c:if>  
                                    </div>
                                </div>
                            </div>
        				...
        				<div class="blog__details__form">
                        	<form action="/revel/help/noticeUpdatePro.rv" name="noticeUpdate" method="post" onsubmit="return noticeValueCheck();">
                            	<div class="row">
                                	<div class="col-lg-6 col-md-6 col-sm-6">
                                		<h4>Title</h4>
                                        <input type="text" name="title" placeholder="Title" value="${notice.title}">
                                        <input type="hidden" name="writer" value="${sessionScope.nick}" />
                                        <input type="hidden" name="noticeNum" value="${notice.index_notice}" />
                                        <input type="hidden" name="pageNum" value="${notice.pageNum}" /> 
                                    </div>
                                    <div class="col-lg-12">
                                    	<h4>Content</h4>
                                        <textarea name="content" placeholder="Content">${notice.content}</textarea>
...
<script>
	// 공지사항 제목, 내용 유효성 검사
	// 빈칸으로 등록하기 버튼을 누르면 알림팝업이 뜨면서 다시 작성해야한다.
	// 제목 또는 내용이 빈칸일 경우 공지사항이 등록되지 않는다. 
	function noticeValueCheck()
	{
		var title = document.noticeUpdate.title.value;
		var content = document.noticeUpdate.content.value;
		
		if(!title)
		{
			alert("제목을 입력해주세요.");
			document.noticeUpdate.title.focus();
			return false;
		}
		...
		{
			alert("내용을 입력해주세요.");
			document.noticeUpdate.content.focus();
			return false;
		}
