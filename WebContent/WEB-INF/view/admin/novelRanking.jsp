...
<%-- 
	revel 소설랭킹 목록 페이지(읽을거다, 읽고있다, 다읽었다, 관심없다)
	http://localhost:8080/revel/manage/novel/ranking.rv 
                    ...
                    <div class="row">
                        ...
                        <div class="col-xl-3 col-md-6 mb-4">
                            <div class="card border-left-primary shadow h-100 py-2">
                                <div class="card-body" onclick="location.href='/revel/manage/novel/ranking.rv?read=read_want'" 
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">
                            	<c:if test="${read eq 'read_want'}">
                            		읽을거다 소설 순위
                            	</c:if>
                            	<c:if test="${read eq 'read_ing'}">
                            		읽고 있다 소설 순위
                            	</c:if>
                            	<c:if test="${read eq 'read_end'}">
                            		다읽었다 소설 순위
                            	</c:if>
                            	<c:if test="${read eq 'read_not'}">
                            		관심없다 소설 순위
                            	</c:if>
                            </h6>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                            	<!-- readRankingCount가 0이면 독서상태별 순위 갯수가 없다. -->
                            	<c:if test="${readRankingCount == 0}">
                            		<p align="center">등록된 순위가 없습니다.</p>
                            	</c:if>
                            	<!-- readRankingCount가 1이상이면 독서상태별 순위를 보여준다 -->
                            	<c:if test="${readRankingCount > 0}">
                                	<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    	<thead align="center">
                                        	<tr>
                                            	...
                                            	<th>장르</th>
                                            	<th>
                                            		<c:if test="${read eq 'read_want'}">읽을거다</c:if>
                            						<c:if test="${read eq 'read_ing'}">읽고 있다</c:if>
                            						<c:if test="${read eq 'read_end'}">다읽었다</c:if>
                            						<c:if test="${read eq 'read_not'}">관심없다</c:if>
                                            	</th>
                                            </tr>
                                    	</thead>
                                  
                                       	<tbody align="center">
                                    		<c:forEach var="readRank" items="${readRankList}">
                                        		<tr>
                                            		<td>${readRank.rank}</td>
                                            		<td>${readRank.title}</td>
                                            		<td>${readRank.writer}</td>
                                            		<td>${readRank.publisher}</td>
                                            		<td>
                                            			<c:if test="${readRank.genre == 1}">로맨스</c:if>
                                            			<c:if test="${readRank.genre == 2}">로맨스 판타지</c:if>
                                            			<c:if test="${readRank.genre == 3}">판타지</c:if>
                                            			<c:if test="${readRank.genre == 4}">현대 판타지</c:if>                             			
                                            			<c:if test="${readRank.genre == 5}">무협</c:if>
                                            		</td>
                                            		<td>
                                            			<c:if test="${read eq 'read_want'}">${readRank.read_want}</c:if>
                            							<c:if test="${read eq 'read_ing'}">${readRank.read_ing}</c:if>
                            							<c:if test="${read eq 'read_end'}">${readRank.read_end}</c:if>
                            							<c:if test="${read eq 'read_not'}">${readRank.read_not}</c:if>
                                            		</td>
                                        		</tr>
                                        	</c:forEach>
                                    	</tbody>
