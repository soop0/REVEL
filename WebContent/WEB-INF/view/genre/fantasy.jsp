...
<%-- 
	revel 판타지 장르 페이지
	http://localhost:8080/revel/genre/fantasy.rv 
--%>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <span onclick="location.href='/revel/genre/fantasy.rv'" style="cursor:pointer;">Fantasy</span>
                    </div>
                </div>
            </div>
        </div>
                        		...
                        		<c:forEach var="fantasy" items="${fantasy}">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${fantasy.image}" onclick="location.href='/revel/detail.rv?bookNum=${fantasy.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${fantasy.comple == 1}">완결</c:if> <c:if test="${fantasy.comple == 0}">연재중</c:if> / ${fantasy.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${fantasy.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${fantasy.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${fantasy.genre == 3}">
                                            				판타지
                                            			</c:if>
                                            		</li>
                                            		<!-- 키워드 나타내기 -->
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == fantasy.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${fantasy.index_book}">${fantasy.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                     ...
                     <c:if test="${count > 0}">
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/genre/fantasy.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/genre/fantasy.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/genre/fantasy.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
