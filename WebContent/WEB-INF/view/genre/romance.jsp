...
<%-- 
	revel 로맨스 장르 페이지
	http://localhost:8080/revel/genre/romance.rv 
--%>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <span onclick="location.href='/revel/genre/romance.rv'" style="cursor:pointer;">Romance</span>
                    </div>
                </div>
            </div>
        </div>
                        		...
                        		<c:forEach var="romance" items="${romance}">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${romance.image}" onclick="location.href='/revel/detail.rv?bookNum=${romance.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${romance.comple == 1}">완결</c:if> <c:if test="${romance.comple == 0}">연재중</c:if> / ${romance.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${romance.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${romance.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${romance.genre == 1}">
                                            				로맨스
                                            			</c:if>
                                            		</li>
                                            		<!-- 키워드 나타내기 -->
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == romance.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${romance.index_book}">${romance.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                    ...
                    <c:if test="${count > 0}">
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/genre/romance.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/genre/romance.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/genre/romance.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
