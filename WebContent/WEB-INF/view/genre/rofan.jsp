...
<%-- 
	revel 로맨스판타지 장르 페이지
	http://localhost:8080/revel/genre/rofan.rv 
--%>
...
<head>
    ...
    <title>Revel | Romance Fantasy | 로맨스 판타지</title>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <span onclick="location.href='/revel/genre/rofan.rv'" style="cursor:pointer;">Romace Fantasy</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="product__page__content">
                        ...
                        <c:if test="${count > 0}">
                        	<div class="row">
                        		<c:forEach var="rofan" items="${rofan}">
                           			<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${rofan.image}" onclick="location.href='/revel/detail.rv?bookNum=${rofan.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${rofan.comple == 1}">완결</c:if> <c:if test="${rofan.comple == 0}">연재중</c:if> / ${rofan.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${rofan.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${rofan.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${rofan.genre == 2}">
                                            				로맨스판타지
                                            			</c:if>
                                            		</li>
                                            		<!-- 키워드 나타내기 -->
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == rofan.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${rofan.index_book}">${rofan.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                     ...
                     <c:if test="${count > 0}">
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/genre/rofan.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/genre/rofan.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/genre/rofan.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
