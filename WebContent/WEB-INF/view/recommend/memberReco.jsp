...
<%-- 
	 revel 개인맞춤 추천페이지
	 http://localhost:8080/revel/recommend/member.rv
	 회원만 이용할 수 있는 서비스(로그인 필수) : aop 처리 페이지 
--%>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <a href="/revel/recommend/member.rv">추천</a>
                        <span onclick="location.href='/revel/recommend/member.rv'" style="cursor:pointer;">개인맞춤 추천</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                        	...
                        	<div class="col-lg-8">
                    			<div class="blog__details__content">
                            		<div class="blog__details__item__text">
                        				<p align="center">추천할 작품이 없습니다. <br />작품을 좀 더 평가하시거나, 선호장르와 선호분량을 조정해주세요!</p>
                        			</div>
                        		</div>
                        	</div>			
                        </c:if>
                        		...
                        		<c:forEach var="reco" items="${reco}">
                           			<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${reco.image}" onclick="location.href='/revel/detail.rv?bookNum=${reco.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${reco.comple == 1}">완결</c:if> <c:if test="${reco.comple == 0}">연재중</c:if> / ${reco.times}화</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${reco.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${reco.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text" >
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${reco.genre == 1}">
                                            				<c:set var="gen" value="romance"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">로맨스</a>
                                            			</c:if>
                                            			<c:if test="${reco.genre == 2}">
                                            				<c:set var="gen" value="rofan"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">로맨스판타지</a>
                                            			</c:if>
                                            			<c:if test="${reco.genre == 3}">
                                            				<c:set var="gen" value="fantasy"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">판타지</a>                                            				<c:set var="gen" value="fantasy"/>
                                            			</c:if>
                                            			<c:if test="${reco.genre == 4}">
                                            				<c:set var="gen" value="mofan"/>
                                            				<a href='/revel/genre/${gen}.rv' style="color: white">현대판타지</a>
                                            			</c:if>
                                            			<c:if test="${reco.genre == 5}">
                                            		...
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == reco.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${reco.index_book}">${reco.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                    ...
                    <!-- 페이지 번호 나타내기 -->
                     <c:if test="${count > 0}">
                    	<fmt:parseNumber var="pageCount" value="${count / pageSize + (count % pageSize == 0 ? 0 : 1)}" integerOnly="ture"/>
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/recommend/member.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/recommend/member.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/recommend/member.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
...
<!-- 
<h1> 개인맞춤추천페이지 </h1>
<c:forEach var="rec100" items="${category100_rank}" begin="0" end="4">
	캐릭터 추천작품 = ${rec100} <br />
</c:forEach>
<c:forEach var="rec200" items="${category200_rank}" begin="0" end="4">
	분위기 추천작품 = ${rec200} <br />
</c:forEach>
<c:forEach var="rec300" items="${category300_rank}" begin="0" end="4">
	소재 추천작품 = ${rec300} <br />
</c:forEach>
<c:forEach var="rec400" items="${category400_rank}" begin="0" end="4">
	독자반응 추천작품 = ${rec400} <br />
</c:forEach>\
 -->