            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <a href="/revel/platform/naverSeries.rv">Recommend</a>
                        <span onclick="location.href='/revel/platform/naverSeries.rv'" style="cursor:pointer;">Naver Series</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="product__page__content">
                        ...
                        <c:if test="${count > 0}">
                        	<div class="row">
                        		<c:forEach var="naverSeries" items="${naverSeries}">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${naverSeries.image}" onclick="location.href='/revel/detail.rv?bookNum=${naverSeries.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${naverSeries.comple == 1}">완결</c:if> <c:if test="${naverSeries.comple == 0}">연재중</c:if>/ ${naverSeries.times}화	</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${naverSeries.review_num}</div>
                                        		<div class="view"><i class="icon_star""></i> ${naverSeries.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${naverSeries.genre == 1}">
                                            				로맨스
                                            			</c:if>
                                            			<c:if test="${naverSeries.genre == 2}">
                                            				로맨스판타지
                                            			</c:if>
                                            			<c:if test="${naverSeries.genre == 3}">
                                            				판타지
                                            			</c:if>
                                            			<c:if test="${naverSeries.genre == 4}">
                                            				현대판타지
                                            			</c:if>
                                            			<c:if test="${naverSeries.genre == 5}">
                                            				무협
                                            			</c:if>
                                            		</li>
                                             		<!-- 키워드 나타내기 -->
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == naverSeries.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${naverSeries.index_book}">${naverSeries.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                    ...
                    <c:if test="${count > 0}">
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/platform/naverSeries.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/platform/naverSeries.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/platform/naverSeries.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
            ...
            <div class="col-lg-6">
                <div class="footer__nav">
                    <ul>
                        ...
                        <li><a href="/revel/gerne/total.rv">장르</a></li>
