            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        ...
                        <a href="/revel/platform/kakaoPage.rv">Platform</a>
                        <span onclick="location.href='/revel/platform/kakaoPage.rv'" style="cursor:pointer;">KakaoPage</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="product__page__content">
                        ...
                        <c:if test="${count > 0}">
                        	<div class="row">
                        		<c:forEach var="kakaoPage" items="${kakaoPage}">
                            		<div class="col-lg-4 col-md-6 col-sm-6">
                                		<div class="product__item">
                                    		<div class="product__item__pic set-bg" data-setbg="${kakaoPage.image}" onclick="location.href='/revel/detail.rv?bookNum=${kakaoPage.index_book}';" style="cursor:pointer;">
                                        		<div class="ep"><c:if test="${kakaoPage.comple == 1}">완결</c:if> <c:if test="${kakaoPage.comple == 0}">연재중</c:if>/ ${kakaoPage.times}화	</div>
                                        		<div class="comment"><i class="icon_chat_alt"></i> ${kakaoPage.review_num}</div>
                                        		<div class="view"><i class="icon_star"></i> ${kakaoPage.rating_revel}</div>
                                    		</div>
                                    		<div class="product__item__text">
                                        		<ul>
                                            		<li style="background-color: RebeccaPurple">
                                            			<c:if test="${kakaoPage.genre == 1}">
                                            				로맨스
                                            			</c:if>
                                            			<c:if test="${kakaoPage.genre == 2}">
                                            				로맨스판타지
                                            			</c:if>
                                            			<c:if test="${kakaoPage.genre == 3}">
                                            				판타지
                                            			</c:if>
                                            			<c:if test="${kakaoPage.genre == 4}">
                                            				현대판타지
                                            			</c:if>
                                            			<c:if test="${kakaoPage.genre == 5}">
                                            				무협
                                            			</c:if>
                                            		</li>
                                            		<!-- 키워드 나타내기 -->
                                            		<c:forEach var="keywordMap" items="${keywordMap}">
                                            			...
                                            			<c:if test="${keyVal == kakaoPage.index_book}">
                                        		...
                                        		<h5><a href="/revel/detail.rv?bookNum=${kakaoPage.index_book}">${kakaoPage.title}</a></h5>
                                    		</div>
                                		</div>
                            		</div>
                            	</c:forEach>
                    ...
                    <c:if test="${count > 0}">
                    	...
                    	<div class="product__pagination">
                    		<c:if test="${startPage > 5}">
                    			<a href="/revel/platform/kakaoPage.rv?pageNum=${startPage-5}"><i class="fa fa-angle-double-left"></i></a>
                    		</c:if>
                        	
                        	<c:forEach var="i" begin="${startPage}" end="${endPage}">
                        		<a href="/revel/platform/kakaoPage.rv?pageNum=${i}">${i}</a>
                        	</c:forEach>
                        	
                        	<c:if test="${endPage < pageCount}">
                        		<a href="/revel/platform/kakaoPage.rv?pageNum=${startPage+5}"><i class="fa fa-angle-double-right"></i></a>
                        	</c:if>
                    	</div>
                    </c:if>
                </div>
